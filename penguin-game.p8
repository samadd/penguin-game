...
function destroyer_of_things(b, set)
  b.solid = false
  b.dead = true
  if b.on_death then b.on_death() end
...
function static_jumpandshoot_input(b)
  local i = {}
  if flr(rnd(50)) == 42 then i.fire1 = true end
  if flr(rnd(50)) == 42 then i.fire2 = true end
  return i
end
...
function make_baddie(x, y, bg, inp, flip, on_death)
...
function make_snowball(x,y,dir)
  ...
  s.update = function()
    s.tick += 1
    if s.tick > 45 then destroyer_of_things(s, projectiles) end
    ...
    if #hits > 0 then
      ...
      if hits[1].health <= 0 then
        destroyer_of_things(hits[1], baddies)
      end
    end
  end
  return s
...
function make_fireball(x,y,dirx, diry)
  local f = {x = x, y = y, vx = dirx * 2.5, width = 1, height = 1, solid = true, vy = diry * 1, accx = 1 , maxvx = 2.5}
  f.bounds = {2,6,2,6}
  f.states = {idle={frames = {55}, modifiers={}, precludes = {}}, walking={frames = {55}, modifiers={}, precludes = {}}}
  f.state = f.states.idle
  f.tick = 0
  f.sprite = f.state.frames[1]
  f.update = function()
    f.tick += 1
    if f.tick > 45 then destroyer_of_things(f, projectiles) end
    local ip = simple_mover_input(f)
    walking_char(f, ip)
    moving_char(f)
    local hits = char_collide(f, {penguin})
    if #hits > 0 then
      del(projectiles, f)
      hit_penguin()
    end
...
function shooting_fire_char(thing, input)
  thing.canfire = max(0, thing.canfire - 1)
  if input.fire2 and thing.canfire == 0 then
    local x, dir
    if thing.flip then x = thing.x - 2 dir = -1 else x = thing.x + 8 dir = 1 end
    add(projectiles, make_fireball(x, thing.y + 4, dir, 0))
    thing.canfire = thing.waitfire
  end
end
...
function do_collisions()
    ...
    if penguin.vy > _gravity_val then
      for hit in all(hits) do
        if bomp_hit(penguin, hit) or penguin.state == penguin.states.sliding then
          ...
          hit.flash = true
          hit.health -= 1
          if hit.health <= 0 then destroyer_of_things(hit, baddies) end
        end
      end
    end
  ...
  local cpoint = checkpoint_hit(penguin)
  if cpoint then spawn_point = cpoint end
  collectable_hit(penguin)
end

function hit_penguin()
  if penguin.health > 0 then
    penguin.immune = 60 penguin.health -=1
  else
    die()
  end
end
...
function draw_thing(t)
  ...
  draw_mods(t)
  if draw then
    if t.flash then whiteout() t.flash = false end
...
badguys = {
  ...
  snowman = {
    states = {idle = {frames = {77}, modifiers = {}, precludes = {}}, jumping = {frames={77}, modifiers = {}, precludes = {}}},
    health = 10,
    updaters = {jumping_char, apply_gravity, moving_char, shooting_fire_char},
    width = 1,
    height = 2,
    jump_power = -3,
    bounds = {0, 7, 0, 15},
...
levels = {
  {
    ...
    baddies = {
      ...
      {x = 986, y = 80, t = badguys.snowman, input = static_jumpandshoot_input, flip = true, on_death = function() mset(126,13,nil) mset(127,13,nil) end}
    },
    platforms = {
...
function start_game()
  ...
  effects = {}
  init_level()
  penguin = make_penguin(spawn_point.x, spawn_point.y, 5, player_input)
  cam_x = 0
  cam_y = 0
...
function init_level()
  for b in all(levels[level].baddies) do
    add(baddies, make_baddie(b.x, b.y, b.t, b.input, b.flip, b.on_death))
  end
  ...
  spawn_point = levels[level].spawn_point
end

function restart()
  penguin.x = spawn_point.x * 8
  penguin.y = spawn_point.y * 8
  penguin.health = 5
end
...

...

...

...

